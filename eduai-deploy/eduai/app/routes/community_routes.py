...
from eduai.app.community_models import (
    ...
    HomeworkQuestion, HomeworkAnswer, CommunityNotification, AppRating,
)
...
RATINGS_ADMIN_EMAIL = "anyanwumichael1800@gmail.com"


@community_bp.route('/ratings/list', methods=['GET'])
def ratings_list():
    """Public — anyone can view ratings + replies, no login required."""
    db = get_db_direct()
    try:
        ratings = db.query(AppRating).filter_by(
            is_public=True
        ).order_by(AppRating.created_at.desc()).limit(100).all()
...
@community_bp.route('/ratings/submit', methods=['POST'])
@require_auth
def ratings_submit(user):
    """Logged-in users submit a star rating + optional comment."""
    data = request.get_json(force=True) or {}
    ...
    try:
        ...
        return jsonify({'success': True, 'id': rating.id})
    except Exception as e:
        db.rollback()
        return jsonify({'error': str(e)}), 500
    finally:
...
@community_bp.route('/ratings/reply', methods=['POST'])
@require_auth
def ratings_reply(user):
    """Admin-only — reply to a rating. Locked server-side to one email."""
    if user.email.lower() != RATINGS_ADMIN_EMAIL.lower():
        return jsonify({'error': 'Not authorized to reply to ratings'}), 403

    data = request.get_json(force=True) or {}
    rating_id  = data.get('rating_id')
    reply_text = (data.get('admin_reply') or '').strip()
    if not rating_id or not reply_text:
        return jsonify({'error': 'rating_id and admin_reply are required'}), 400

    db = get_db_direct()
    try:
        rating = db.query(AppRating).filter_by(id=rating_id).first()
        if not rating:
        ...
        db.commit()
        return jsonify({'success': True, 'admin_reply': reply_text})
    except Exception as e:
        db.rollback()
        return jsonify({'error': str(e)}), 500
    finally:
